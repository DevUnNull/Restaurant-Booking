...
public class ReviewRepositoryImpl implements ReviewRepository {
    ...
    public List<Review> findAllApproved() {
        List<Review> reviews = new ArrayList<>();
        String sql = "SELECT r.*, u.full_name FROM Reviews r JOIN Users u ON r.user_id = u.user_id WHERE r.status = 'APPROVED' ORDER BY r.created_at DESC";
        try (Connection conn = DatabaseUtil.getConnection();
            ...
            while (rs.next()) {
                ...
                review.setUserName(rs.getString("full_name")); // Lấy tên người dùng
                reviews.add(review);
            }
        } catch (SQLException e) {
            e.printStackTrace();
